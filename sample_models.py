...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    deep_rnn = GRU(units, 
                   ...
                   name='rnn_0')(input_data)
    deep_rnn = BatchNormalization(name='bn_0')(deep_rnn)
    for i in range(recur_layers-1):
        deep_rnn = GRU(units, 
                   return_sequences=True,
                   recurrent_dropout=0.2,
                   name=f'rnn_{i+1}')(deep_rnn)
        deep_rnn = BatchNormalization(name=f'bn_{i+1}')(deep_rnn)        
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(GRU(units, 
                                  return_sequences=True, 
                                  recurrent_dropout=0.2))(input_data)
    # Add a TimeDistributed(Dense(output_dim)) layer
